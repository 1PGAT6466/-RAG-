...
def _stage_classify(ctx: dict):
    from .ingest import _auto_classify, _auto_folder
    from src.storage.db import update_file_category, update_file_folder
    ...
    folder = _auto_folder(ctx["filename"], ctx["text"])
    if folder:
...
def _stage_images(ctx: dict):
    ...
    Feature Flag: RAG_IMAGE_EXTRACT（默认开）
    """
    try:
        from config import RAG_IMAGE_EXTRACT
