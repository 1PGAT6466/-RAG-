...
# 标准分类知识表：根据标准号前缀判断其领域类别，用于建「标准→领域」语义
# 前缀匹配（前缀→领域类别），命中即归类，未命中归为"通用"
STANDARD_CATEGORY_RULES = [
    # 基础/电气/汽车/德标（前缀明确，放最前）
    (r'^GB/T\s*\d\.', '基础标准'),      # GB/T 1.x 标准化导则
    (r'^IEC\s*', '电工标准'),
    (r'^DIN\s*', '德国标准'),
    (r'^QC/T\s*', '汽车标准'),
    (r'^ISO\s*', '国际标准'),
...
def classify_standard(name: str) -> str | None:
    """标准号领域分类：根据前缀规则返回领域类别，未命中返回 None"""
    if not name:
        return None
    name = name.strip()
...
# 标准分类的合法领域集合（供 LLM 精分类约束输出）
STANDARD_DOMAINS = ["材料", "紧固件", "工艺", "机械制图", "电工", "密封件", "轴承", "公差配合", "其他"]
