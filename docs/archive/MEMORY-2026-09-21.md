...
## 🔴 SeedDMS 文档预览固化（2026-09-17）

用户要求 DMS 支持文档预览（Word/Excel/PDF），并固化持久化。已完成：

**镜像固化**：
- 新建镜像 `seeddms-fuxi:latest`（4.89GB），基于 `usteinm/seeddms:latest` + LibreOffice 25.2 + unoconv shim
- 容器 `seeddms` 已用新镜像重建（同端口 8080→80、同 bind mount、restart=unless-stopped）
- 重建脚本：`E:\更新RAG框架\scripts\seeddms_recreate.bat`
- ⚠️ 不 commit 的话容器重建后 LibreOffice 会丢；已 commit，未来重建用 `seeddms-fuxi:latest`

**⚠️ 关键坑：SeedDMS 生效配置文件不是容器内路径**：
- 实际生效配置 = bind mount 的宿主机文件 `E:\测试项目\SeedDMS\conf\settings.xml` → 容器内 `/var/lib/seeddms/conf/settings.xml`
- 由 env `SEEDDMS_CONFIG_FILE=/var/lib/seeddms/conf/settings.xml` 指定
- **`/home/www-data/seeddms60x/conf/settings.xml` 是另一个不生效的同名文件（inode 不同），改它会白改**（本次踩过）
- 所有 settings.xml 修改必须针对 `/var/lib/seeddms/conf/settings.xml`

**预览必备三项配置**（都在生效的 settings.xml 里）：
1. `<display ... convertToPdf="true">`（原 false）
2. `<extension name="pdfviewer" disable="false"><parameter name="__disable__">0` （原 true/1）
3. 新增 `<converters target="pdf">` 段（8 个 Office mimeType → `unoconv -f pdf --stdout -v '%f'`）

**unoconv shim**（`/usr/local/bin/unoconv`）：bash 脚本，把 unoconv 调用翻译为 `libreoffice --headless --convert-to pdf`。SeedDMS 已有 preview 转换器命令用 unoconv，装 shim 后无需改这些命令。

**预览入口**：SeedDMS Web UI → 点文档名进详情页 → 下方「PDF 预览」iframe（`op.PdfPreview.php` → pdf.js viewer）。

**端口/访问**：SeedDMS `http://172.25.30.11:8080`（admin/admin）；伏羲 `http://172.25.30.11:8099`。

**⚠️ SeedDMS 6.0.41 不支持 Select/MultiSelect 自定义属性**：属性类型只有 整数/浮点/字符串/布尔/日期/Email/URL/Folder/Document/User/Group。需要下拉枚举只能「字符串+正则约束」或换 DMS（Teedy/Paperless-ngx/Mayan EDMS 原生支持）。

### 表格预览（Excel 大数据集）（2026-09-17）

**问题**：Excel→PDF 对大表不可用。实测「2026年1月-6月采购数据_供应商分类.xlsx」（doc 103，18519 行×15 列）→ LibreOffice 转 PDF 得到 **1080 页 / 65MB**，pdf.js 加载慢且用户以为「数据不全」。

**解决**：新增自定义页面 `op/op.TablePreview.php`（容器内 `/home/www-data/seeddms60x/seeddms-6.0.41/op/`）：
- 用容器内已有的 **PhpSpreadsheet**（`/home/www-data/seeddms60x/vendor/phpoffice/phpspreadsheet`，autoload 路径 `/home/www-data/seeddms60x/vendor/autoload.php`）读 xlsx
- 服务端分页（每页 1000 行）+ 全列展示，全量数据可翻页查看
- 入口：`class.ViewDocument.php` 中，对 xlsx/xls/ods/csv mimeType 加按钮「📊 在新页面查看完整表格数据」→ `op/op.TablePreview.php?documentid=X&version=Y`
- 登录字段坑：SeedDMS 登录 POST 到 `/op/op.Login.php`，字段名是 **`login`** 和 **`pwd`**（不是 user）；session cookie 名 `mydms_session`
- DocumentContent 读内容方法名是 **`content()`**（不是 getContent）；大小 `getFileSize()`；路径 `getPath()`（相对 contentDir）

**预览相关改动都已在镜像 `seeddms-fuxi:latest` 里（已 commit）**。
